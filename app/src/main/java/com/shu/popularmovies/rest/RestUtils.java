...
public class RestUtils {
    ...
    public final static String MOVIE_DB_BACKDROP_PATH = "https://image.tmdb.org/t/p/w780";

    public static final String dbUserKey = BuildConfig.MOVIE_DB_USER_API_KEY;
    ...
    public static FavMovieEntry convertToMovieEntry(Movie movieData) {
        ...
        Iterator<Integer> i =  movieData.getGenreIds().iterator();
    ...
    public static Movie convertToMovie(FavMovieEntry movieData) {
        ...
        if (movieData.getVideo() == 1) entry.setVideo(true);
        else entry.setVideo(false);
        ...
        if (movieData.getAdult() == 1) entry.setAdult(true);
        else entry.setAdult(false);
